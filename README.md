...
4. Path Variable in Both User Variables and System Variables should contains the path to the bin directories of the Java and Maven, something like the following:
...
### Folders
  ...
  * testng.xml
  ...
  Properties files will contain the values in Key/Value Pairs. E.g. you can see in the below screenshot ServerURL for the Api is defined as “ServerURL” key and it’s value is “https://reqres.in/”. 

  #### 3. reports:
  As the name suggests it will contain all of the reports for the tests run. File Names are formatted the following way: 
...
> “{MethodName} - {year}_{month}_{day}_{hours(24h)}_{minutes}_{seconds}_{milliseconds}”
  ...
  #### 7. pom.xml:
  This file holds all of the information about the Compiler, Dependencies, Build, Project Details.

  #### 8. testng.xml:
  This File contains all of the details of the test classes, groups, listeners and Any parameters.

### Packages
  #### 1. main:
  This Package have following Folders:
...
### ApiConfigs - EndPoints:
  All of the Endpoints for the particular Module or Feature are defined in this folder. Suggested Interface for the Endpoint files so that the values of the EndPoints cannot be changed in other classes. By Default Variables of interface are final.
...
For Example:
 ...
 > DemoData: This interface has all of the data variables required to perform the test.
  
  Like: userNameLogin field has the information of the UserName for logIn screen.

### pageEvents:

This package will contain the Classes which will have all of the events which will be used for testing the feature or page.

Recommended to use “Events” postfix in the class name for clarification

For Example:

 > DemoPageEvents: It has all of the events in Methods form which will be used to test the Demo page.
...
### pageObjects:
  ...
  This package will contain the Classes which will have all of the locators of the page or feature which need to be tested.

  > Recommended to use “Elements” postfix in the class name for clarification

  For Example:

  > DemoPageElements: It has all of the locators for the Demo page.
...
#### 1. CommonMethods:
  As the name suggests it will contain all of the common methods which can be used for any kind of help. 

  Like: TakeScreenshot() Method: It will take the screenshot and will need a single parameter of MethodName.

#### 2. Constants:

  This Interface will have all of the constants for the project. You can use this interface for the environment variables.
